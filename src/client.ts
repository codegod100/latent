...
let SERVERS: any[] = []
let serverSessions: Map<string, { token: string, expires: string, isModerator: boolean }> = new Map()
...
function setupWebSocket() {
  ...
  ws.onopen = () => {
    const s = serverSessions.get(currentServer.url);
    if (s && new Date(s.expires) > new Date()) ws?.send(JSON.stringify({ type: 'auth', token: s.token }));
  };
  ...
  ws.onmessage = (event) => {
    try {
      ...
      if (data.type === 'new_message') {
        ...
        alert('You have been banned from this server.'); location.reload();
      }
...
async function authenticateWithServer(server: any) {
  ...
  const submit = async (nonce: string | null = null) => {
    try {
      ...
      if (res.ok) { serverSessions.set(server.url, { token: data.token, expires: data.expiresAt, isModerator: !!data.isModerator }); log(`Authenticated with ${server.host}`) }
...
async function refreshMessages(beforeId: string | null = null) {
  ...
  try {
    ...
    const headers: any = {}; const s = serverSessions.get(currentServer.url);
...
function renderMessages(shouldScrollBottom = true) {
    ...
    return `
      <div class="msg-item" id="msg-${m.id}">
        ...
        <div class="msg-actions">
          ...
          ${isModerator() ? `<div class="action-btn ban" onclick="window.banUser('${m.did}', '${m.handle}')" title="Ban User">🚫</div>` : ''}
        </div>
  ...
  if (shouldScrollBottom) { container.scrollTop = container.scrollHeight; const jumpBtn = document.getElementById('jump-to-present'); if (jumpBtn) jumpBtn.style.display = 'none'; }
}

// --- SEARCH & NAVIGATION ---
(window as any).clearSearch = () => { const input = document.getElementById('search-input') as HTMLInputElement; if (input) { input.value = ''; } const resultsEl = document.getElementById('search-results'); const clearBtn = document.getElementById('clear-search'); if (resultsEl) resultsEl.style.display = 'none'; if (clearBtn) clearBtn.style.display = 'none'; };
...
(window as any).performSearch = async (query: string) => {
  ...
  try {
    const searchTasks = SERVERS.filter(s => !s.error).map(async (server) => { try { const res = await fetch(`${server.url}/api/search?q=${encodeURIComponent(query)}`); return { server, results: res.ok ? await res.json() : [] } } catch (e) { return { server, results: [] } } })
...
async function serverMutation(server: any, endpoint: string, body: any, method: string = 'POST') {
  ...
  const submit = async (nonce: string | null = null) => {
    ...
    if (method !== 'GET' && method !== 'HEAD') fetchOpts.body = JSON.stringify({ ...body, accessToken: tokens.access_token, dpopProof: dpop, pdsUrl, requesterDid: session.did });
    const res = await fetch(`${server.url}${endpoint}`, fetchOpts); const data = await res.json();
    if (data.isChallenge) return submit(data.dpopNonce); return { ok: res.ok, status: res.status, data };
  };
  return await submit();
}
...
async function showApp(session: any) {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (profile.handle) {
        currentUserHandle = profile.handle; document.getElementById('user-handle')!.textContent = `@${profile.handle}`;
        await syncServersFromPds(session); renderAdminUI();
        const params = new URLSearchParams(window.location.search); const inviteCode = params.get('invite');
        if (inviteCode) {
          const submitJoin = async (nonce: string | null = null) => {
            const dpop = await getDpopProof(session, 'GET', probeUrl, nonce);
            const res = await fetch(`${currentServer.url}/api/join`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ accessToken: tokens.access_token, dpopProof: dpop, pdsUrl, did: session.did, code: inviteCode }) });
            const data = await res.json(); if (data.isChallenge) return submitJoin(data.dpopNonce);
            if (res.ok) { serverSessions.set(currentServer.url, { token: data.token, expires: data.expiresAt, isModerator: !!data.isModerator }); alert(`Successfully joined ${currentServer.name}!`); const cleanUrl = new URL(window.location.href); cleanUrl.searchParams.delete('invite'); window.history.replaceState({}, '', cleanUrl.toString()); refreshMessages(); } else { alert(`Failed to join: ${data.error || 'Invalid code'}`); }
          };
          await submitJoin();
        }
      }
...
async function hydrateServers() {
  ...
  SERVERS = await Promise.all(SERVER_URLS.map(async (url) => {
    try { const res = await fetchWithTimeout(`${url}/api/meta`, { timeout: 10000 }); if (!res.ok) throw new Error(); const meta = await res.json(); const host = new URL(url).host; return { ...meta, url, host, id: meta.id || host } }
...
if (msgList) {
  msgList.onscroll = () => {
    if (msgList.scrollTop < 10 && currentMessages.length > 0 && hasMoreMessages && !isLoadingOlder) { refreshMessages(currentMessages[currentMessages.length - 1].id); }
    const jumpBtn = document.getElementById('jump-to-present');
    if (jumpBtn) { if (msgList.scrollHeight - msgList.scrollTop - msgList.clientHeight > 500) { jumpBtn.style.display = 'block'; } else { jumpBtn.style.display = 'none'; } }
  };
}
